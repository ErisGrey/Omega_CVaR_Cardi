...
void Solver::createModel() {
	...
	for (int i : N_st)
	{
		...
		masterModel.add(0.005 * lam - (1 - k[i]) * BIGM <= x[i]);
	}

	IloExpr exp5(masterEnv);
	for (int i : N_st)
	{
		exp5 += k[i];
	}
	masterModel.add(exp5 <= 50);
...
void Solver::write_output()
{
	...
	if (instance->index == 0)
	{
		ocsv << "Window" << "," << "Return" << "," << "Omega" << "," << "Min weight" << "," << "Max weight" << endl;
	}
	ocsv << "window " << instance->index << ","
		<< sum << "," << masterCplex.getObjValue() << "," << min_weight << "," << max_weight << endl
		<< std::flush;
	ocsv.close();
}